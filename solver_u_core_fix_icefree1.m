function [u_s, noIceIndex] = solver_u_core_fix_icefree1(visc_s, visc, AGlen_s, para)
% BETA VERSION
...
if strcmpi(type_LBC,'zero')
    ...
    % noIceIndex = find(H_s<=Hmin);
    % if ~isempty(noIceIndex)
    %     emptyNodeNum = length(noIceIndex);
    %     for enn=1:emptyNodeNum
    %         Ms_tmp = noIceIndex(enn);
    %         for j=1:N
    %             LHS((Ms_tmp-1)*N+j,:) = 0; % deepseek
    %             LHS((Ms_tmp-1)*N+j,(Ms_tmp-1)*N+j) = 1;
    %             RHS((Ms_tmp-1)*N+j,1) = 0; % sia(j,Ms_tmp); % %sia(j,Ms_tmp);
    %         end
    %     end
    % end
...
%%
% deepseek
% Before solving, add small diagonal term for stability
LHS = LHS + 1e-10*speye(size(LHS));
...
%% deepseek
% Add physical constraints to velocities
u_s(u_s < -1e4) = 0;  % Remove extremely negative velocities
u_s(u_s > 1e4) = 1e4;  % Cap extremely high velocities

% Ensure off-ice areas are truly zero
u_s(:,H_s <= Hmin) = 0;

u_s = smooth_velocities(u_s, para);
