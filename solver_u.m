function [u_s] = solver_u(visc_s, visc)
...
switch type_BBC
    ...
    case 2 % Coulomb friction law (taub = sigma_xz)
        Cb = 0.84*m_max;
        for i = 1:Ms
            EP(i) = epr(i)*rho*g*H_s(i);
            if iter_u == 1
                ubi = 0;
            else
                ubi = u_s_lst(1,i);
            end
            Lb_CFL(i) = 4*AGlen_s(1,i)*H_s(i)*dzeta*Cb(i)^n*EP(i)^n*m_max(i)/...
                (ubi*m_max(i) + Cb(i)^n*EP(i)^n*lambda_max(i)*AGlen_s(1,i)); % /(1+(H_s(i)/W_s(1,i))^2)
        end
        frozeni = 0;
        for i=2:Ms-1
...
%         for i = 1:Ms
%             Lb1_LFL(i) = 4*dzeta*H_s(i)*dhBdx_s(i)/dx/(1-4*H_s(i)*dhBdx_s(i)*dzetadx_s(1,i));
%             Lb2_LFL(i) = 2*dzeta*H_s(i)*beta2_s(i)/visc_s(1,i)/(1-4*H_s(i)*dhBdx_s(i)*dzetadx_s(1,i));
%         end
