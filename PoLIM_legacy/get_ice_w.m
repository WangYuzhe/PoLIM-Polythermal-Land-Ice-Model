...
for i = 2:M-1
    for j = 2:N
        w_vs(j,i) = u(1,i)*dhBdx(i) - H(i)*(U(1,i)/2 + sum(U(2:j,i)))*dzeta;
        w_vs(1,i) = u(1,i)*dhBdx(i) - H(i)*(U(1,i)/2 + 0*sum(U(1:2,i)))*dzeta;
        w_vs(j,1) = u(1,1)*dhBdx(1) - H(1)*(U(1,1)/2 + sum(U(2:j,1)))*dzeta;
        w_vs(j,M) = u(1,M)*dhBdx(M) - H(M)*(U(1,M)/2 + sum(U(2:j,M)))*dzeta;
    end
end
w_vs(1,1) = u(1,1)*dhBdx(1) - H(1)*(U(1,1)/2 + 0*sum(U(1:2,1)))*dzeta;
w_vs(1,M) = u(1,M)*dhBdx(M) - H(M)*(U(1,M)/2 + 0*sum(U(1:2,M)))*dzeta;
%
%--------------------------------------------------------------------------
for i = 2:M-1
    w(:,i) = [u(1,i)*dhBdx(i); (w_vs(1:end-1,i)+w_vs(2:end,i))/2];
end
